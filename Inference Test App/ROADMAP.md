...
**Note:** This roadmap was written for the proof-of-concept test app (`android_app/`). Many Phase 2 and Phase 4 goals have been achieved in the production app — see [`Medlens/README.md`](../medlens/README.md).

**Current status:** Proof-of-concept test app — superseded by the production MedLens app.

## Current State (v0.1 — Test App)
- ✅ BiomedCLIP INT8 ONNX inference — 126 ms avg, 83.6 MB
- ✅ MedGemma Q4_K_S generation — 7.8 tok/s, 2.2 GB
- ✅ Streaming text output with live tok/s display
- ✅ Stop generation support
- ✅ Permission handling for external storage
- ✅ Two-tab UI (BiomedCLIP image analysis, MedGemma text generation)
...
- Single screen: capture/pick image → auto-analyze → show clinical summary  
  **Done** — unified chat interface
- Pass BiomedCLIP embedding as context to MedGemma prompt  
  **Done** — zero-shot classification via cosine similarity (30 conditions, 512-dim embeddings)
- Template: *"Given this medical image analysis [embedding summary], provide clinical assessment for: [user question]"*  
  **Done** — `MedicalClassifier` builds structured prompts from classification results
...
## Known Limitations
- **Memory:** 2.2 GB model + KV cache + OS leaves ~2 GB free on 12 GB device. Devices with 8 GB RAM may struggle.
- **No GPU offload:** Adreno 735 GPU is unused. Vulkan backend would help significantly.
- **No multi‑turn:** Each generation is independent — no conversation memory yet. ✅ Fixed in MedLens
- **Model download:** Users must manually push models via `adb` or file manager.
- **Single device tested:** Only validated on Realme GT Neo 6 (Snapdragon 8s Gen 3).
